#!/usr/bin/env ruby
# scripts/extract_v8.rb
#
# Extraction Sonnet 4.5 + scoring Vigil'Asso pour chaque PDF JOAFE
# téléchargé pour le sample BODACC v8.
#
# Lance via : bundle exec rails runner scripts/extract_v8.rb
#
# Source PDFs   : data/pdfs_positifs_v8/*.pdf
# Output CSV    : data/scores_positifs_v8.csv (schéma identique à
...
# Plafond coût  : $35 sur Sonnet 4.5 (input $3/MTok, output $15/MTok).
...
PDFS_DIR     = File.join(PROJECT_ROOT, 'data/pdfs_positifs_v8')
OUTPUT_CSV   = File.join(PROJECT_ROOT, 'data/scores_positifs_v8.csv')
LOG_PATH     = '/tmp/extract_v8.log'
...
abort "#{PDFS_DIR} introuvable. Lance d'abord fetch_jo_for_bodacc.rb." unless Dir.exist?(PDFS_DIR)
...
# ─── Appel API direct (anthropic gem) avec mesure usage ─────────────
...
def call_sonnet_with_usage(pdf_path)
  ...
  begin
    ...
    [nil, usage_h || { input_tokens: 0, output_tokens: 0 }, "JSON invalide: #{e.message[0..150]}"]
    ...
    [nil, { input_tokens: 0, output_tokens: 0 }, "#{e.class}: #{e.message[0..200]}"]
  end
end

# ─── Helper scoring (style phase4_run.rb) ───────────────────────────

...
WHERE = 'familleavis="collective" AND commercant like "association" AND dateparution>="2020-01-01" AND dateparution<"2023-01-01"'
PAGE_SIZE = 100
OUTPUT = "bodacc_associations_positifs_2020_2022.csv"
...
puts "[info] #{total} annonces 2020-2022 à récupérer"
...
CSV.open(OUTPUT, "w", write_headers: true, headers: headers) do |csv|
  ...
  loop do
    ...
    offset += PAGE_SIZE
    puts "[progress] #{[offset, total].min}/#{total}" if offset % 500 == 0
